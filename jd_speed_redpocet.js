...
function reward_query() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_query", {
      "inviter": ["6gVLS2FR8-9dVTWOYwO1NA"][Math.floor((Math.random() * 1))],
      linkId
    }), async (err, resp, data) => {
      try {
        if (err) {
...
async function redPacket() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_receive",{"inviter":["6gVLS2FR8-9dVTWOYwO1NA"][Math.floor((Math.random() * 1))], linkId}),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
...
function invite() {
  let t = +new Date()
  let inviterId = [
    "8pyaXcdcZBq+UIAcS4kMWg==",
    "JUg7pC0yDA8Cz5YiO3luBOZ7SCsIo/cKz9RsnyIs3rE="]
