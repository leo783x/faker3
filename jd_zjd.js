...
!(async() => {
    ...
    $.authorTuanList = await getAuthorShareCode('https://raw.githubucaonimasaaaasercontent.com/Aaron-lv/updateTeam/master/shareCodes/jd_zz.json');
    if (!$.authorTuanList) {
        $.http.get({ url: 'https://purge.jsdeubucaonimasaaaaserclivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_zz.json' }).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
        await $.wait(1000)
        $.authorTuanList = await getAuthorShareCode('https://cdn.jsdelubucaonimasaaaasercivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_zz.json') || [];
    }
    $.authorTuanList = [];
    // await getRandomCode();
...
async function getRandomCode() {
    await $.http.get({ url: `http://go.chisbsbsbsbang.fun/read/zuan/${randomCount}`, timeout: 10000 }).then(async(resp) => {
        if (resp.statusCode === 200) {
            try {
                let { body } = resp;
                body = JSON.parse(body);
