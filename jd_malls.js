...
if ($.isNode()) {
  ...
  actURL = process.env.ACT_URL || 'https://cdn.jsdelivr.net/gh/JoahZhang/mny@main/mall_active.json'
} else {
...
function getCodeList(url) {
  return new Promise(resolve => {
      ...
      $.get(options, async (err, resp, data) => {
		  data = "{"pro": ["vwnzgCl,vtgaMqm,vd2CvIv","vtn6ig0,vI2QVVN,vd2QUQH","vInqSvR,vI2G0iv,vd2QaWw","vtnGr22,vI2Q6Ga,vC2DaIB","vCny1Su,vM2Qpx7,vwgDTBA","vInnLfX,vL2QUCh,vt2Cfzm","vtnzMz7,vL2DI33,vwgahYV","vwnvj32,vMg1d9A,vt28gic"],"prodev":["nLhu1LP,nt5zCmM,vK2G0vq"]}"
          try {
              if (err) {
                  // $.log(err)
                  $.getCodeListerr = false
