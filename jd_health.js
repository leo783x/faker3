...
const inviteCodes = ["T019xJ8QNHFBoQ-tYhrwkvACjVfnoaW5kRrbA","T0225KkcREhN_VKGcUmnxfNbcACjVfnoaW5kRrbA"]
...
function runTimes() {
  return new Promise((resolve, reject) => {
    $.get({
      url: `https://api.jdshar******ecode.xyz/api/runTimes?activityId=health&sharecode=${$.code}`
    }, (err, resp, data) => {
      if (err) {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({
      url: `https://api.jdsha*******recode.xyz/api/health/${randomCount}`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
